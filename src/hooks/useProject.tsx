...
function mapDbRebarToConfig(rc: any): RebarConfig {
  return {
    ...
    hInsetIn: Number(rc.h_inset_in ?? 3),
    ...
    vInsetIn: Number(rc.v_inset_in ?? 3),
    ...
    gridInsetIn: Number(rc.grid_inset_in ?? 3),
    gridWastePct: Number(rc.grid_waste_pct),
    gridTotalLf: Number(rc.grid_total_lf) || 0,
    // L-Bar — nullable DB columns fall back to makeDefaultRebar values
    lbarEnabled: rc.lbar_enabled ?? false,
    lbarBarSize: (rc.lbar_bar_size ?? "#4") as RebarConfig["lbarBarSize"],
    lbarSpacingIn: Number(rc.lbar_spacing_in ?? 12),
    lbarVerticalFt: Number(rc.lbar_vertical_ft ?? 0),
    lbarVerticalIn: Number(rc.lbar_vertical_in ?? 0),
    lbarBendLengthIn: Number(rc.lbar_bend_length_in ?? 12),
    lbarOverlapIn: Number(rc.lbar_overlap_in ?? 12),
    lbarInsetIn: Number(rc.lbar_inset_in ?? 3),
    lbarWastePct: Number(rc.lbar_waste_pct ?? 0),
    lbarTotalLf: Number(rc.lbar_total_lf) || 0,
  };
}
  ...
  return {
    ...
    sections: sections
      ...
      .map((s: any): CalcSection => ({
        ...
        lengthFraction: s.length_fraction ?? "0",
        widthFt: Number(s.width_ft),
        widthIn: Number(s.width_in),
        widthFraction: s.width_fraction ?? "0",
      ...
      for (const area of committedAreas) {
        ...
        if (area.sections.length > 0) {
          const sectionRows = area.sections.map((s) => ({
            ...
            length_fraction: s.lengthFraction ?? "0",
            width_ft: s.widthFt,
            width_in: s.widthIn,
          ...
          const { error: rebarErr } = await supabase.from("rebar_configs").upsert(
            {
              ...
              grid_inset_in: config.gridInsetIn,
              grid_waste_pct: config.gridWastePct,
              grid_total_lf: 0,
              lbar_enabled: config.lbarEnabled,
              lbar_bar_size: config.lbarBarSize,
              lbar_spacing_in: config.lbarSpacingIn,
              lbar_vertical_ft: config.lbarVerticalFt,
              lbar_vertical_in: config.lbarVerticalIn,
              lbar_bend_length_in: config.lbarBendLengthIn,
              lbar_overlap_in: config.lbarOverlapIn,
              lbar_inset_in: config.lbarInsetIn,
              lbar_waste_pct: config.lbarWastePct,
              lbar_total_lf: 0,
            },
